...
export default function PremiumUpgradePage() {
  ...
  // Fungsi memvalidasi voucher dari database
  const handleApplyVoucher = async (forcedCode?: string) => {
    ...
    try {
      const { data: voucherData, error } = await supabase
        .from('vouchers')
        .select('*')
        .eq('code', code) // 🟢 DISESUAIKAN: Menggunakan kolom 'code' sesuai database Anda
        .maybeSingle();
      ...
      if (voucherData) {
        const discount = Number(voucherData.discount_value) || 0;
        setDiscountPercent(discount);
        setFinalAmount(basePrice - (basePrice * discount) / 100);
          ...
          <div className="bg-white border border-slate-200 shadow-xl rounded-2xl p-6 sm:p-8 space-y-6 text-xs animate-in fade-in slide-in-from-bottom-4 duration-200">
            ...
            <div className="space-y-4 border-y border-slate-100 py-5 text-slate-700 leading-relaxed">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-slate-300 rounded-xl focus:outline-none focus:border-blue-500 text-xs font-mono uppercase"
                    />
              ...
              <div className="pt-3 border-t border-slate-100">
                <span className="font-bold text-slate-900 block mb-2">🎟️ Daftar Voucher Tersedia :</span>
                
                {dbError && (
                  <div className="p-2 mb-2 bg-red-50 text-red-700 rounded border border-red-200 text-[11px]">
                    {dbError}
                  </div>
                )}

                {availableVouchers.length === 0 ? (
                  <p className="text-slate-400 italic text-[11px]">Tidak ada voucher promo aktif saat ini.</p>
                ) : (
                  <div className="grid grid-cols-2 gap-2">
                    {availableVouchers.map((v) => (
                      <div 
                        key={v.id || v.voucher_code} 
                        onClick={() => handleApplyVoucher(v.voucher_code)}
                        className={`p-2 border rounded-lg cursor-pointer transition-all flex justify-between items-center ${
                          appliedVoucher === v.voucher_code 
                            ? 'bg-blue-50 border-blue-500 shadow-sm' 
                            : 'border-dashed border-slate-300 bg-slate-50 hover:bg-blue-50 hover:border-blue-400'
                        }`}
                      >
                        <span className="font-mono font-bold text-blue-700 uppercase tracking-wide">{v.voucher_code}</span>
